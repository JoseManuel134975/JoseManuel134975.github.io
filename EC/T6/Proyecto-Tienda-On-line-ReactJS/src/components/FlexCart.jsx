...
export default function FlexCart({ cart, setCart }) {
  ...
  useEffect(() => {
    const prices = cart.cartProducts.map((element) => element.regularPrice * element.quantity);
  ...
  return (
    <>
      <section className="mt-5 d-lg-flex flex-lg-column justify-content-lg-center align-items-lg-center gap-3">
        {cart.cartProducts.length > 0 &&
          cart.cartProducts.map((product) => (
            <article
              className="d-lg-flex flex-lg-row justify-content-lg-between align-items-lg-center gap-3"
              key={product.id}
            >
